...
const MAX_SKILLS = 10;
const MENTOR_SCORING_SCRIPT_PATH = path.join(__dirname, '..', 'scripts', 'mentor_scoring.py');
const HAS_MENTOR_SCORING_SCRIPT = fs.existsSync(MENTOR_SCORING_SCRIPT_PATH);
...
function runPythonMentorScoring(payload) {
  if (!HAS_MENTOR_SCORING_SCRIPT) {
    throw new Error('Mentor scoring script unavailable');
  }

  const scriptPath = MENTOR_SCORING_SCRIPT_PATH;
  const commands = [
    ['python', [scriptPath]],
    ['py', ['-3', scriptPath]],
    ['py', [scriptPath]],
